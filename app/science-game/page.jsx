...
/* ------------------------ SCIENCE 1000+ ENGINE (LOCAL, FAST) ------------------------ */
...
// class-wise topics (syllabus)
const scienceSyllabusByClass = (cls) => {
  const c = Number(cls)
  if (c <= 2) return ['living', 'plants', 'earth', 'body']
  if (c <= 5) return ['living', 'plants', 'earth', 'body', 'matter', 'energy']
  if (c <= 7) return ['living', 'plants', 'earth', 'body', 'matter', 'energy', 'force', 'space']
  return ['living', 'plants', 'earth', 'body', 'matter', 'energy', 'force', 'space', 'chem', 'env']
}

const sciFacts = {
  gases: {
    airMost: { q: 'Which gas is most abundant in air?', a: 'Nitrogen', wrong: ['Oxygen', 'Carbon dioxide', 'Hydrogen'] },
    plantsTake: { q: 'Plants take in which gas for photosynthesis?', a: 'Carbon dioxide', wrong: ['Oxygen', 'Nitrogen', 'Helium'] },
    weBreathe: { q: 'Humans breathe in mainly to get ____.', a: 'Oxygen', wrong: ['Nitrogen', 'Carbon dioxide', 'Steam'] },
  },
  temps: {
    boil: { q: 'Water boils at ____ °C (at sea level).', a: '100', wrong: ['0', '50', '80'] },
    freeze: { q: 'Water freezes at ____ °C.', a: '0', wrong: ['10', '50', '100'] },
  },
  body: {
    pump: { q: 'Which organ pumps blood?', a: 'Heart', wrong: ['Lungs', 'Stomach', 'Brain'] },
    breathe: { q: 'Which organ helps us breathe?', a: 'Lungs', wrong: ['Liver', 'Kidney', 'Skin'] },
    brain: { q: 'Which organ controls the body?', a: 'Brain', wrong: ['Heart', 'Stomach', 'Bone'] },
  },
  earth: {
    tempTool: { q: 'Which instrument measures temperature?', a: 'Thermometer', wrong: ['Barometer', 'Compass', 'Scale'] },
    rain: { q: 'Water falling from clouds is called ____.', a: 'Rain', wrong: ['Smoke', 'Sand', 'Dust'] },
    wind: { q: 'Moving air is called ____.', a: 'Wind', wrong: ['Cloud', 'Rain', 'Fog'] },
  },
  matter: {
    solid: { q: 'Which is a solid?', a: 'Ice', wrong: ['Air', 'Steam', 'Water'] },
    liquid: { q: 'Which is a liquid?', a: 'Water', wrong: ['Stone', 'Smoke', 'Air'] },
    gas: { q: 'Which is a gas?', a: 'Air', wrong: ['Ice', 'Wood', 'Soil'] },
  },
}

const makeNumericMCQ = ({ q, answer, deltas = [-10, -5, 5, 10] }) => {
  const a = Number(answer)
  const wrongs = uniq(
    deltas
      .map((d) => fmtNum(Math.max(0, Math.round((a + d) * 100) / 100)))
      .filter((x) => x !== fmtNum(a))
  ).slice(0, 3)

  while (wrongs.length < 3) wrongs.push(fmtNum(randInt(0, Math.max(20, a + 20))))

  return { q, answer: fmtNum(a), options: shuffle([fmtNum(a), ...wrongs].slice(0, 4)) }
}

const generateScienceQuestion = (topic, cls, difficulty) => {
  const c = Number(cls)
  const d = clamp(Number(difficulty || 1), 1, 10)

  if (topic === 'living') {
    const livingThings = ['Dog', 'Cat', 'Tree', 'Bird', 'Fish', 'Human']
    const nonLiving = ['Rock', 'Chair', 'Table', 'Book', 'Bottle', 'Stone']
    const a = pick(livingThings)
    const wrong = shuffle([pick(nonLiving), pick(nonLiving), pick(nonLiving)]).slice(0, 3)
    const q = pick(['Which one is living?', 'Which is a living thing?', 'Select the living object:'])
    return { q, answer: a, options: shuffle([a, ...wrong]) }
  }

  if (topic === 'plants') {
    const pool = [
      sciFacts.gases.plantsTake,
      { q: 'Plants need ____ to make food.', a: 'Sunlight', wrong: ['Plastic', 'Smoke', 'Noise'] },
      { q: 'Green plants make food by the process called ____.', a: 'Photosynthesis', wrong: ['Respiration', 'Evaporation', 'Condensation'] },
    ]
    const item = pick(pool)
    return { q: item.q, answer: item.a, options: shuffle([item.a, ...item.wrong]) }
  }

  if (topic === 'earth') {
    const pool = [sciFacts.earth.tempTool, sciFacts.earth.rain, sciFacts.earth.wind]
    const item = pick(pool)
    return { q: item.q, answer: item.a, options: shuffle([item.a, ...item.wrong]) }
  }

  if (topic === 'body') {
    const pool = [sciFacts.body.pump, sciFacts.body.breathe, sciFacts.body.brain]
    const item = pick(pool)
    return { q: item.q, answer: item.a, options: shuffle([item.a, ...item.wrong]) }
  }

  if (topic === 'matter') {
    const mode = pick(['solid', 'liquid', 'gas'])
    const item = sciFacts.matter[mode]
    return { q: item.q, answer: item.a, options: shuffle([item.a, ...item.wrong]) }
  }

  if (topic === 'energy') {
    const forms = ['Sound', 'Heat', 'Light', 'Electrical']
    const q = pick(['Which is a form of energy?', 'Select an energy type:', 'Which one is energy?'])
    const a = pick(forms)
    const wrong = shuffle(['Stone', 'Sand', 'Wood', 'Water', 'Soil', 'Plastic']).slice(0, 3)
    return { q, answer: a, options: shuffle([a, ...wrong]) }
  }

  if (topic === 'force') {
    const type = pick(['friction', 'pushpull', 'speed'])
    if (type === 'friction') {
      return { q: 'Friction acts ____ to motion.', answer: 'Opposite', options: shuffle(['Opposite', 'Along', 'Upwards', 'Randomly']) }
    }
    if (type === 'pushpull') {
      return { q: 'Pushing or pulling is called ____.', answer: 'Force', options: shuffle(['Force', 'Shadow', 'Sound', 'Heat']) }
    }

    // Speed MCQ (bigger variety)
    const distance = randInt(20, 200 + d * 20)
    const time = randInt(2, 20 + d)
    const speed = Math.round((distance / time) * 10) / 10
    return makeNumericMCQ({
      q: `A body travels ${distance} m in ${time} s. Speed = ? (m/s)`,
      answer: speed,
      deltas: [-3, -1, 1, 3],
    })
  }
  ...
  if (topic === 'space') {
    const pool = [
      { q: 'The Sun is a ____.', a: 'Star', wrong: ['Planet', 'Comet', 'Asteroid'] },
      { q: 'The Earth is a ____.', a: 'Planet', wrong: ['Star', 'Galaxy', 'Comet'] },
      { q: 'Which gives light and heat to Earth?', a: 'Sun', wrong: ['Moon', 'Mars', 'Jupiter'] },
    ]
    const item = pick(pool)
    return { q: item.q, answer: item.a, options: shuffle([item.a, ...item.wrong]) }
  }
  ...
  if (topic === 'chem') {
    const pool = [
      sciFacts.gases.airMost,
      { q: 'pH less than 7 means the solution is ____.', a: 'Acidic', wrong: ['Neutral', 'Basic', 'Salty'] },
      { q: 'pH equal to 7 means the solution is ____.', a: 'Neutral', wrong: ['Acidic', 'Basic', 'Hot'] },
      { q: 'Which is a greenhouse gas?', a: 'Carbon dioxide', wrong: ['Oxygen', 'Nitrogen', 'Helium'] },
    ]
    const item = pick(pool)
    return { q: item.q, answer: item.a, options: shuffle([item.a, ...item.wrong]) }
  }
  ...
  if (topic === 'env') {
    const pool = [
      { q: 'Planting more trees helps reduce ____.', a: 'Air pollution', wrong: ['Rainfall', 'Sunlight', 'Gravity'] },
      { q: 'Which one is biodegradable?', a: 'Paper', wrong: ['Plastic', 'Glass bottle', 'Metal can'] },
      { q: 'Using public transport can reduce ____.', a: 'Pollution', wrong: ['Oxygen', 'Soil', 'Temperature always'] },
    ]
    const item = pick(pool)
    return { q: item.q, answer: item.a, options: shuffle([item.a, ...item.wrong]) }
  }
  ...
  // fallback
  return { q: sciFacts.temps.boil.q, answer: sciFacts.temps.boil.a, options: shuffle([sciFacts.temps.boil.a, ...sciFacts.temps.boil.wrong]) }
}

const makeScienceQuestionsEngine = (classLevel, count, enabledTopics) => {
  const c = Number(classLevel)
  const topics = (enabledTopics?.length ? enabledTopics : scienceSyllabusByClass(c)).filter((t) => SCI_TOPICS[t])

  const qs = []
  for (let i = 0; i < count; i++) {
    const topic = pick(topics.length ? topics : scienceSyllabusByClass(c))
    const diff = 1 + Math.floor(i / 3)
    const item = generateScienceQuestion(topic, c, diff)
    qs.push({
...
/* ------------------------ TEACHER SETTINGS ------------------------ */
const SETTINGS_KEY = 'science_teacher_settings_v1'
...
const defaultTeacherSettings = (cls) => ({
  ...
  enabledTopics: scienceSyllabusByClass(cls),
})
...
export default function ScienceGame() {
  ...
  useEffect(() => {
    ...
    const safe = {
      ...
      timePerQuestion: clamp(Number(s.timePerQuestion || 25), 10, 90),
      ...
      enabledTopics: (s.enabledTopics?.length ? s.enabledTopics : scienceSyllabusByClass(cls)).filter((t) => SCI_TOPICS[t]),
    }
    setSettings(safe)
  ...
  useEffect(() => {
    ...
    setSettings((prev) => ({
      ...prev,
      enabledTopics: prev.enabledTopics?.length ? prev.enabledTopics : scienceSyllabusByClass(cls),
    }))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cls])
  ...
  const startGame = async () => {
    ...
    try {
      ...
      setTimeLeft(settings.timePerQuestion || 25)

      const topics = (settings.enabledTopics?.length ? settings.enabledTopics : scienceSyllabusByClass(cleanCls)).filter(
        (t) => SCI_TOPICS[t]
      )

      const qs = makeScienceQuestionsEngine(cleanCls, count, topics)
      setQuestions(qs)
      setStep('game')
    } finally {
      setLoading(false)
  ...
  const submitAnswer = (timeout = false) => {
    ...
    const isCorrect = !timeout && selectedOption != null && String(selectedOption) === String(q.answer)
  ...
  const saveTeacher = () => {
    const safe = {
      ...
      timePerQuestion: clamp(Number(settings.timePerQuestion || 25), 10, 90),
      ...
      enabledTopics: (settings.enabledTopics?.length ? settings.enabledTopics : scienceSyllabusByClass(cls)).filter((t) => SCI_TOPICS[t]),
    }
    setSettings(safe)
      ...
      <div className="min-h-screen bg-[url('/gamebg.png')] bg-cover flex items-center justify-center p-6 relative">
        ...
        <div className="w-full max-w-6xl grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white/95 p-8 rounded-3xl shadow-xl">
            <h2 className="text-2xl font-extrabold mb-2">🔬 Science Quiz</h2>
            <p className="text-sm text-gray-600 mb-5">
              One play per day. Weekly points accumulate. Speed + accuracy matter.
            </p>
                ...
                {mistakes.map((m, i) => (
                  <div key={i} className="bg-red-50 border border-red-200 rounded-xl p-3">
                    <div className="text-xs text-gray-600 mb-1">
                      Topic: <b>{SCI_TOPICS[m.topic] || m.topic}</b>
                    </div>
        ...
        <div className="lg:col-span-2 bg-white/95 rounded-2xl p-6 shadow text-center">
          <div className="flex items-center justify-between mb-3">
            <div className="text-sm font-extrabold">
              Topic: <span className="text-slate-700">{SCI_TOPICS[q?.topic] || q?.topic}</span>
            </div>
          ...
          <div className="text-2xl md:text-3xl font-extrabold mb-5">{q?.q}</div>
